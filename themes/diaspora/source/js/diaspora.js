        ...
        switch (true) {
			...
			case (tag.indexOf('switchsearch') != -1):
                ...
                var searchInterval = setInterval(function() {
                    if ($('#local-search-input').length > 0 && $('#local-search-result').length > 0) {
            ...
            case - 1 != tag.indexOf("comment"):
                if ($('#waline-container').length > 0 && walineConfig) {
                    if (walineConfig.enable) {
                        Diaspora.loading();
                        $('#waline-container').removeClass('comment').removeClass('link');
                        Waline.init(Object.assign({}, walineConfig, { el: '#waline-container' }));
                        Diaspora.loaded();
                    } else {
                        $('#waline-container').removeClass('comment').removeClass('link');
                        $('#waline-container').html("评论已关闭");
                    }
                }
                return false;
                break;
